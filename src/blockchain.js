...
const { generateHash, decodeData, parseJSON } = require("../util");
...
class Blockchain {
  ...
  _addBlock(block) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        reject(new Error("An error happened while adding the block"));
      }
    });
  }
  ...
  submitStar(address, message, signature, star) {
    ...
    try {
      return new Promise(async (resolve, reject) => {
        //1. Getting the time from the message
        ...
        //5. Creating the block and add it to the chain
        let block = new BlockClass.Block({
          data: { address, signature, message, star }
        });
